...
function AppContent() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          // Don't default to admin on error for security reasons.
          // If we can't fetch the profile, we can't determine the role.
          // We will rely on the cached profile if offline.
          // If there's no cached profile, they will be blocked.
          setOwnerId(null);
          setRole(null);
        } finally {
          setIsLoading(false);
  ...
  useEffect(() => {
    const seedPaints = async () => {
      ...
      const allProducts = [...asianPaints, ...nerolacPaints].map(p => ({ ...p, userId: user.uid }));
      
      for (const product of allProducts) {
        const exists = await db.products.where('sku').equals(product.sku).and(p => p.userId === user.uid).first();
        if (!exists) {
          await db.products.add(product as any);
        }
      }
      
      localStorage.setItem(`paints_seeded_${user.uid}`, 'true');
    };
    if (user) seedPaints();
  }, [user]);
